...
function header()
 {
     $this->Image('letterhead-header.png',10,10,200);
 }
 ...
 function footer()
 {
     ...
     $this->Image('letterhead-footer.png',0,275,221);
 }
...
function addInvoice( $ref )
{
    ...
    //$this->RoundedRect($r1, $y1, ($r2 - $r1), $y2, 3.5, 'D');
    //$this->Line( $r1, $mid, $r2, $mid);
    $this->SetXY( $r1 + ($r2-$r1)/2 - 12, $y1 );
    $this->SetTextColor(46,116,181);
    $this->SetFont( "Arial", "", 10);
    $this->Cell(0,10, "INVOICE ", 0, 1, "L");
    $this->SetXY( $r1 + ($r2-$r1)/2 , $y1);
    $this->SetTextColor(46,116,181);
    $this->SetFont( "Arial", "", 10);
    ...
    // $this->Cell(10,50, "INVOICE", 0, 1, "C");
    // $this->SetXY( 150,30 );
    // $this->SetFont( "Arial", "", 12);
    // $this->Cell(80,50,$ref, 0,0, "C");
}
...
function addTo($tab)
{
    ...
    $this->SetXY( $r1, $y1 );
    $this->SetFont("","I","12");
    $this->Cell($r2, 4, "TO", '', '1', "R");
    //$this->Rect( $r1, $y1, $r2, $y2, "D");
    $this->Line( $r1, $y1+6, $r1+$l2, $y1+6);
    $colX = $r1;
    //$colonnes = $tab;
    while ( list( $lib, $pos ) = each ($tab) )
    {

        $this->SetXY( $colX, $y1+10);
        if($lib == 1)
        {
            $this->SetFont("","U","10");
            $this->SetTextColor(46,116,181);
            $this->Cell( $r2, 1, $pos, 0, 1, "R");
            $this->SetTextColor(0,0,0);
        }
        else
        {
    ...
    $this->Line( $r2+$r1, $y1-24, $r2+$r1, $y1+8);
    $this->SetFont("","","");
...
function addFor($tab)
{
    ...
    $this->SetXY( $r2, $y1 );
    $this->SetFont("","I","12");
    $this->Cell($r2, 4, "FOR", '', '1', "L");
    $colX = $r1;
    while ( list( $lib, $pos ) = each ($tab) )
    {
        $this->SetXY( $colX + 15, $y1+10);
        $this->SetFont("","I","10");
        $this->Cell( $r2, 1, $lib, 0, 1, "L");
        $y1 += 5;
    }
...
function addCols( $tab )
{
    ...
    $this->Line( $r1, $y1+$y2, $r1+$r2, $y1+$y2);
}
...
function addSubTotal($amount)
{
    ...
    $this->SetXY($r2,$y2);
    $this->Cell(18,10,'Sub Total ',0,0,'R');
    $this->SetFont("Arial","I","10");
    $this->SetXY($r2+20,$y2);
    $this->Cell(0,10,$amount,0,0,'L');
}
...
function addVat($amount)
{
    ...
    $this->SetXY($r2,$y2);
    $this->Cell(18,1,'V.A.T ',0,0,'R');
    $this->SetFont("Arial","B","11");
    $this->SetXY($r2+20,$y2);
    $this->Cell(0,1,$amount,0,0,'L');
    $this->Line( $r2-5, $y2+3, $r2+40, $y2+3);

}

function addTotalDue($amount)
{
    $r1  = 10;
    $r2  = $this->w - ($r1 * 5) ;
    $y1  = -14;
    ...
    $this->SetFont("Arial","B","11");
    $this->SetXY($r2,$y2);
    $this->Cell(18,10,'Total Due ',0,0,'R');
    $this->SetFont("Arial","B","11");
    $this->SetXY($r2+20,$y2);
    $this->Cell(0,10,$amount,0,0,'L');
}
